...
// 仅限服务端调用：删除冲突用户（同时删 Auth �?users 表）
export async function POST(request: NextRequest) {
  try {
    const { username, userId } = await request.json()
    if (!username && !userId) {
      return NextResponse.json({ success: false, error: '需�?username �?userId' }, { status: 400 })
    }

    let targetId = userId as string | undefined

    // 如果只给�?username，先�?users 表拿 id；查不到再去 auth 用户元数据里�?    if (!targetId && username) {
    ...
    // 删除 users 表记录（即使不存在也忽略�?    if (username) {
